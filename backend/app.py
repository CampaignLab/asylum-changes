...
def email():
    try:
        ...
        if not postcode:
            return error_response("No postcode provided", status_code=400)
        ...
        if response.status_code != 200:
            return error_response(
                error_message=response.json().get(
                    "error", "Error fetching constituency data"
                ),
        ...
        if not constituency:
            return error_response(
                "Could not determine parliamentary constituency for this postcode",
                status_code=404,
            )
        ...
        if not mp_email:
            return error_response(
                f"No MP email found for constituency: {constituency}", status_code=404
            )

        return Response(
            body={"email": mp_email, "name": mp_name},
            status_code=200,
            headers={"Content-Type": "application/json"},
        )
...
@app.route(
    "/transcribe", methods=["POST"], cors=True, content_types=SUPPORTED_CONTENT_TYPES
)
def transcribe():
    ...
    try:
        ...
        with open(temp_file_path, "rb") as audio_temp:
            ...
            try:
                transcription = openai_client.audio.transcriptions.create(
                    model="gpt-4o-transcribe", file=audio_temp
                )
        ...
        try:
            ...
            app.log.info(
                f"{submission_id}: Warning: Failed to clean up temporary file {temp_file_path}: {str(e)}"
            )
...
def extract_json_from_response(response_text):
    ...
    try:
        ...
        app.log.info(
            f"JSONDecodeError: {e.msg} pos {e.pos} line {e.lineno} col {e.colno}"
        )
        if hasattr(e, "pos"):
            app.log.error(
                f"Character at error position: {repr(json_text[e.pos - 5:e.pos + 5])}"
            )
            app.log.error(
                f"Hex dump around error: {[hex(ord(c)) for c in json_text[e.pos - 5:e.pos + 5]]}"
            )
        else:
...
def apply_prompt_to_transcript(template):
    ...
    try:
        ...
        if not transcript:
            ...
            return Response(body={"error": "Transcript required"}, status_code=400)
